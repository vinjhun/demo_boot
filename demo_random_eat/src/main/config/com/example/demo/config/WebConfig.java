...
@Configuration
@Description("The configuration must be within the main folder (same with the main class)")
...
//	}
	...
	@Bean
	@Description("ServletContextTemplateResolver is repalced with spring resource template resolver")
	public ITemplateResolver templateResolver(ServletContext sc) {

		SpringResourceTemplateResolver tr = new SpringResourceTemplateResolver();

		tr.setTemplateMode(TemplateMode.HTML);
		tr.setPrefix("templates/thymeleaf/"); // for maven, it reside in webapps, [can add WEB-INF] ?
		tr.setSuffix(".html");
	...
	@Override
	@Description("Because spring boot cant recognize the folder in static, it requires the mapping")
	...
	@Override
	@Description("can add handler interceptor for every httpservlet request and response")
